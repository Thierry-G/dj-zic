...
function scpUploads($mode){
    global $remotes;
    $cmd = "sudo -u thierry /usr/local/bin/sync-uploads.sh";
...
if (isset($_GET['cmd'])) {
    ...
    switch ($_GET['cmd']) {
        ...
        case 'setMode':
            if (isset($_GET['status']) &&  ( $_GET['status']=='start' || $_GET['status']=='pause' || $_GET['status']=='stop' ) ) {
                $dataStream = getFileJsonData($STREAM);
                $dataStream['stream']['status'] = $_GET['status'];
