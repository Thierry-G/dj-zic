...
class SoundMonitor:
    ...
    def monitorSound(self):
        ...
        while True:
            try:
                data = arecord_proc.stdout.read(self.Silence_Detector.CHUNK * 4)  # Read CHUNK samples, 2 bytes per sample, 2 channels
                if not data:
...
if __name__ == "__main__":
    ...
    monitor = SoundMonitor(enable_logging=not args.no_logging)
    monitor.monitor()